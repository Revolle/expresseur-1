...
void c_note::compile(int ipartNr)
{
	partNr = ipartNr;
	if ( chord)
		chord_counter ++ ;
...
{
	f->Write(wxString::Format("<sound %s=\"%s\" />", name,value));
}
...
void c_measure_sequence::compile(int partNr)
{
	switch (type)
	{
	case t_note: ((c_note*)(pt))->compile(partNr); break;
		//case t_harmony: ((c_harmony*)(pt))->compile(); break;
		//case t_backup: ((c_backup*)(pt))->compile(); break;
		//case t_forward: ((c_forward*)(pt))->compile(); break;
		//case t_barline: ((c_barline*)(pt))->compile(); break;
		//case t_direction: ((c_direction*)(pt))->compile(); break;
	default: break;
	}
}
...
c_measure::c_measure(wxXmlNode *xmlnode)
{
	...
	while (child)
	{
		...
		else if (name == "barline")
		{
			...
			attributes = new c_attributes(child);
		child = child->GetNext();
	}
}
c_measure::c_measure(const c_measure &measure , bool withContent)
{
	...
	if (measure.attributes != NULL)
		attributes = new c_attributes(*(measure.attributes), withContent);
	if (withContent)
	{
...
void c_measure::compile(c_measure *previous_measure , int partNr)
{
	if (attributes )
	{
		division_quarter = attributes->divisions;
	}
	if (attributes && (attributes->mtime))
	{
		switch (attributes->mtime->beat_type)
		{
		case 1:
			division_beat = 4 * division_quarter;
			division_measure = 4 * division_quarter * attributes->mtime->beats;
			break;
		case 2:
			division_beat = 2 * division_quarter;
			division_measure = 2 * division_quarter * attributes->mtime->beats;
			break;
		case 8 :
			division_beat = (3 * division_quarter) / 2;
			division_measure = division_quarter * attributes->mtime->beats / 2;
			break;
		case 4:
		default :
			division_beat = division_quarter;
			division_measure = division_quarter * attributes->mtime->beats;
			break;
		}
	}
	else
	...
	{
		key_fifths = attributes->key->fifths;
	}
	else
	{
		if (previous_measure)
...
void c_part::compile(int ipartNr)
{
	chord_counter = 0;
	partNr = ipartNr;
	...
	{
		c_measure *current_measure = *iter;
		current_measure->compile(previous_measure, partNr);
