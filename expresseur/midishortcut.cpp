...
: wxDialog(parent, id, title, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
	...
	wxFlexGridSizer *topsizer = new wxFlexGridSizer(1, wxSize(5, 5));
...
void midishortcut::getMidiinDevices()
{
	...
	while (true)
	{
		...
		{
			wxString s;
			s.sprintf("%s %s:%s", _("invalid"), SMIDI, ch);
			nameDevice.Add(s);
		}
		nbDevice++;
	}
...
{
	long nbSelector = mConf->writeFile(lfile, CONFIG_SHORTCUTNB, 0);
	for (int nrSelector = 0; nrSelector < nbSelector; nrSelector++)
	{
...
{
	long nbSelector = mConf->readFile(lfile, CONFIG_SHORTCUTNB,0);
	for (int nrSelector = 0; nrSelector < nbSelector; nrSelector++)
	{
...
bool midishortcut::hitkey(wxChar c , bool on , wxFrame *parent)
{
	...
	{
		if (valueKey[nrSelector].Contains(valueHitkey))
		{
			int pos = valueKey[nrSelector].Find(valueHitkey);
			...
			if (on)
			{
				...
				wxString strace;
				strace.Printf("shortcut %c : NoteOn %s %s , p=%d , v=%d", c, 
			...
			{ // off
				if (valueEvent[nrSelector] == snoteonoff)
				{
					int p = valueMin[nrSelector];
					int v = 0;
					int pos = valueKey[nrSelector].Find(c);
					int len = valueKey[nrSelector].length();
					if (valueMin[nrSelector] != valueMax[nrSelector])
					{
						if (len > 1)
					...
					wxString strace;
					strace.Printf("shortcut %c : NoteOff %s %s , p=%d , v=%d", c, 
						(nrDevice >= -1) ? nameDevice[nrDevice + 1] : nameDevice[0],
						(nrChannel >= -1) ? nameChannel[nrChannel + 1] : nameChannel[0],
						p, v);
					parent->SetStatusText(strace);
				...
				{
					wxString strace;
					strace.Printf("shortcut %c : off", c);
					parent->SetStatusText(strace);
				}
				valueHitkey = 0;
			}
	...
	{
		wxString strace;
		strace.Printf("shortcut %c : not recognized", c);
		parent->SetStatusText(strace);
	}
	return found;
}
