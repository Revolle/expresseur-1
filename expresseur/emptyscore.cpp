...
bool emptyscore::setFile(const wxFileName & WXUNUSED(lfilename), bool onstart)
{
	wxClientDC dc(this);
	dc.Clear();
	dc.DrawText(_("No image score"), 20, 20);
	if ((onstart == false) && (mConf->get(CONFIG_EMPTYSCOREWARNING, 1) == 1))
	{
...
void emptyscore::setPosition(int WXUNUSED(pos), bool WXUNUSED(playing), bool WXUNUSED(wuick))
{
}
