...
--========================
function dumpPitch(t,name)
--========================
  -- print the list of pitches in table t, for name ( form lit chord, scale, penta ) 
  local l = t[name]
  local spres = name .. ": "
  for i = -7 , 7 do
    if l[i] then
      spres = spres .. "(" .. ptos(l[i][-1][1]) .. ")"
      for j = 1 , #l[i][0] do
        if j == 2 then
          spres = spres .. "+"
        end
        if l[i][0][j] then
          spres =  spres .. ptos(l[i][0][j])
        end
      end
      if i == -1 then
        spres = spres .. " <"
...
--=============================================
  ...
  elseif string.find(smodifier,"0") then  -- semi-diminue mb57   
    ...
    elseif string.find(smodifier,"sus[4]*") then
      degreeInChord[3] = 6
    elseif string.find(smodifier,"[-m]*") and string.find(isChord,"[-m]") then -- m isChord case sensitive
      degreeInChord[3] = 4
    elseif string.find(smodifier,"m[^7]*") and string.find(isChord,"M[^7]*") then -- m isChord case sensitive
      degreeInChord[3] = 5
    end   
    -- quinte
    ...
    --septieme
    if string.find(smodifier,"maj7") or ( string.find(smodifier,"m7") and string.find(isChord,"M7")) then
      degreeInChord[7] = 12
...
--[[
  ...
  for i,c in ipairs(stest) do
    if c.schord then
      ...
      local r 
      if stest[i+1] then
        r = E.stringToChord(c.schord  , stest[i+1].schord)
      else
        r = E.stringToChord(c.schord  , stest[i+1].schord)
      end
      print("tone=" .. ptos(currentTone - 1), "root=" .. ptos(r.root - 1 ), "bass=" .. ptos(degreeToPitch(r.root,r.bass,0)) .. ">" .. ptos(degreeToPitch(r.root,r.nextBass,0)) , "chord","penta","scale",r.text)
      ...
      dumpPitch(r.pitch,"bass")
      dumpPitch(r.pitch,"chord")
      dumpPitch(r.pitch,"penta")
      dumpPitch(r.pitch,"scale")
    end
  end
