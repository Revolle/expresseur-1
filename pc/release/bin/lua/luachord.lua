...
Chords cannot be mixed with free-text in a line.
...
--[[
-- uncomment this zone for standalone local LUA debugger test
...
local function extractParts()
  ...
  while nbest ~= nil do
    ...
    if ( string.find(pat[nbest].name,",") ) then
      -- thera are additional information in the line
...
local function extractChord()
  ...
  -- chords are recognized if the line contains only chords
  local nrChord = 1
  for nrSection=1 , #section , 1 do
    ...
    while(endl == false) do
      ...
      local bufchord = {}
      local sl = string.sub(section[nrSection].content,pl,l)
      local gstart
      local gend = 0 
      local sg
      ...
      while(true) do
        ...
        if (( sg ~= "[" ) and ( sg ~= "]" ) 
          ...
          if interpretedChord then
            table.insert(bufchord,{})
            local tchord = bufchord[#(bufchord)]
            tchord.interpretedChord= interpretedChord
            tchord.posStart = section[nrSection].posStart + gstart + pl - 2
            tchord.posEnd = section[nrSection].posStart + gend + pl - 2
          else
            -- a line with free-text is ignored
            bufchord = {}
            break 
          end
        end
      ...
      pl = l
      -- move buffered chords off the line in the section
      for b = 1 , #bufchord , 1 do
        table.insert(section[nrSection].chord,{})
        local pc = section[nrSection].chord[#(section[nrSection].chord)]
        pc.interpretedChord = bufchord[b].interpretedChord
        ...
        pc.posStart = bufchord[b].posStart
        pc.posEnd = bufchord[b].posEnd
        pc.nrChord = nrChord
        nrChord = nrChord + 1
      end
    end
...
-- set the current chord as specified in the string parameter
  currentChord = texttochord.stringToChord(chordText)
end
...
function playPitches(bid,velocity,index,black,scale,track,pstart,pend,delay,decay)
  -- play the pitches
...
function E.playScaleChord(time,bid,ch,nr,velocity,param,index,mediane,whiteindex,whitemediane,black)
  playPitches(bid,velocity,whitemediane,black,"chord","scale",1,-1,0,values["scale decay"])
end
function E.playScalePenta(time,bid,ch,nr,velocity,param,index,mediane,whiteindex,whitemediane,black)
  playPitches(bid,velocity,whitemediane,black,"penta","scale",1,-1,0,values["scale decay"])
end
function E.playChordUp(time,bid,ch,nr,velocity,param,index,mediane,whiteindex,whitemediane,black)
  playPitches(bid,velocity,1,0,"chord","chord",-1,1,values["chord delay"],values["chord decay"])
end
...
function E.playBackground(time,bid,ch,nr,velocity,param,index,mediane,whiteindex,whitemediane,black)
  playPitches(bid,velocity,1,0,"chord","background",1,-1,0,45)
  playPitches(bid,velocity,1,0,"bass","bass",1,1,0,0)
end
...
function E.playWalkingBass(time,bid,ch,nr,velocity,param,index,mediane,whiteindex,whitemediane,black)
  playPitches(bid,velocity,whiteindex,black,"bass","bass",1,1,0,0)
end
...
--[[
-- uncomment this zone to test the module locally 
-- uncoment also the zone at the beginning of this script, to simulate the environment

--print("stringtodegree=",texttochord.stringToDegree("c",1))
--print("stringtopitch=",texttochord.stringToPitch("c4",1))
  E.setFile("mysong.txt")
  ...
  --print("TEST POSITION")
  --E.setPosition(20)
  --print("20:getPosition" , E.getPosition())
  --print("20:getTextPosition" , E.getTextPosition())
  --E.setPosition(40)
  --print("40:getPosition" , E.getPosition())
  --print("40:getTextPosition" , E.getTextPosition())
  ...
  local sp = 1
  while (true) do
    sstart , send = E.getRecognizedScore()
    if sstart and ( sstart ~= -1 ) then
      if ( sstart > 0 ) then
        print ( sstart , send , string.sub(score,sstart,send))
      end
    else
      break ;
    end
  ...
  local n = 1
  local  pitch 
  local nrChord , nrSection , ch
  E.firstPart()
  while ( true ) do
    if ( currentChord == nil ) then print("end of song") break end
    local chPart , chSection , chChord = E.getTextPosition()
    print(chPart ,chSection, chChord )
    pitch =  E.getIndexPitches("chord",-1,0)
    print ("                                 chord -1 pitch" , table.unpack(pitch) )
    pitch =  E.getIndexPitches("chord",0,0)
    print ("                                 chord 0  pitch" , table.unpack(pitch) )
    pitch =  E.getIndexPitches("chord",1,0)
    print ("                                 chord 1  pitch" , table.unpack(pitch) )
    pitch =  E.getIndexPitches("bass",1,0) -- walking bass index is [1..4]
    print ("                                 bass pitch" , table.unpack(pitch) )
    E.nextChord()
    n = n + 1 
    if ( n == 49 ) then 
      E.nextPartSmooth()
      print ( ">> next Part Smooth" )
    end
    if ( n > 100 ) then break end
  end
  print("RESTART with high level function and luafake.lua module")
  E.firstPart()
  n = 1 
  while ( true ) do
    if ( currentChord == nil ) then print("end of song") break end
    --E.playBass(time,bid,ch,nr,velocity,param,index,mediane,whiteindex,whitemediane,black)
    print(">>on background")
    E.playBackground(1,1,1,1,64,"",1,1,1,1,0)
    luabass.dumpListMidi()
    print(">>of background")
    E.playBackground(2,1,1,1,0,"",1,1,1,1,0)
    luabass.dumpListMidi()
    print(">>on chord up")
    E.playChordUp(3,3,1,3,64,"",1,1,1,1,0)
    luabass.dumpListMidi()
    print(">>of chord up")
    E.playBackground(4,3,4,1,0,"",1,1,1,1,0)
    luabass.dumpListMidi()
    print(">>change chord")
    E.changeChordOn(3,2,1,2,64,"",1,1,1,1,0)
    luabass.dumpListMidi()
    print(">>on background")
    E.playBackground(1,1,1,1,64,"",1,1,1,1,0)
    luabass.dumpListMidi()
